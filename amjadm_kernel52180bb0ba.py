...
get_ipython().run_cell_magic('time', '', "run_simpleCNN(net, optimizer, criterion, epoch = 10, best_eval_loss = b_loss)\nepoch, b_loss, ehist, thist = resume(net, optimizer, fn = 'simpleCNN-best.pth.tar')\nreport(net, evalData)")




submission_03 = pd.DataFrame({'img':testData.driver_imgs_list.iloc[:, 0], 
...
for i  in tqdm.tnrange(len(testData)):
        x = testData[i]
        # I have to add one extra axis at the beginning by None
        submission_03.at[i, 1:] = tocpu(net(togpu(x[None,...])))[0].detach().numpy()
        
    
print(submission_03)
Print("The Submission file has been made!")




submission_03.to_csv('submission_03_Last.csv', index = False)




print(os.listdir("../working")) , b_loss
