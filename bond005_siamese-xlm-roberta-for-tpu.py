...
def do_submit(data_for_training: Dict[str, Tuple[Tuple[np.ndarray, np.ndarray], np.ndarray]],
    ...
    print('Duration of the training procedure must be less '           'than {0} seconds.'.format(max_duration))
    print('')
    ...
    gc.collect()
    end_time = time.time()
    print('Duration of the final submission for language '          '"{0}" is {1:.3f} seconds'.format(
        language_for_validation, end_time - start_time))
    print('Postprocessing duration (after training) is '           '{0:.3f} seconds.'.format(end_time - postprocessing_start_time))
...
print('Languages in the labeled trainset: '       '{0}'.format(all_languages_for_training))
