...
#what is "TfidfVectorizer" and what is the "ngram_range"
#it converts the text documents to a matrix of tfidf features
#tfidf features are the method to convert the textual information into the vector space, they are a measure of how important a word in a text is
#what is ngram_range
#what is ngram
#ngram is the number of words in a sequence
#ngram_range is the lower and upper range of the features to be extracted
#the ngram_range() gives the upper range and the lower range of the ngram size thst should be considered in the features of the corpus
#the tfidf_vec.fit() , fits in the corpus of document extracts the features, and then form the vectorised dataset in the vector space
#the tfidf_vec.transform(), takes the corpus provided as the input and then extract all the features that are already present in the tfidr_vec and ignores all
...
cnt_srs = train_df['desc_nwords'].value_counts()
cnt_srs
...
#what does the label encoder fit does
for col in cat_vars:
...
def run_lgb(train_X, train_y, val_X, val_y,alpha):
    ...
    evals_result = {}
    model = lgb.train(params, lgtrain, 1000, valid_sets=[lgval], early_stopping_rounds=10000, verbose_eval=20, evals_result=evals_result)
...
#parametertuning, increased the training dataset by 10 times
...
# Training the model #
learningList = [0.001,0.003,0.009,0.027,0.081,0.243,0.729,2.178,6.514,19.51]
#the best in above list is 0.243
train_error = list()
test_error = list()
for i in learningList:
    print("the learning_rate 0f ",i)
    model, evals_result = run_lgb(dev_X, dev_y, val_X, val_y, i)
    pred_train = model.predict(dev_X, num_iteration=model.best_iteration)
    pred_val= model.predict(val_X,num_iteration = model.best_iteration)
    train_error.append(metrics.mean_squared_error(dev_y,pred_train))
    test_error.append(metrics.mean_squared_error(val_y,pred_val))
    del model, evals_result
...
plt.plot(learningList,train_error)
plt.plot(learningList,test_error)
plt.show()
...
axes = plt.gca()
axes.set_xlim([min(learningList),max(learningList)])
axes.set_ylim([min(train_error),max(test_error)])
plt.plot(learningList,train_error)
plt.plot(learningList,test_error)
plt.show()
