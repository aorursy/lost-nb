...
train_date_list = train_data.iloc[:, 5].unique()
...
len(test_data.groupby(['Province/State', 'Country/Region']))
...
for idx, (k, v) in enumerate(train_data.groupby(['Province/State', 'Country/Region'])):
    #if k != ('NULL', 'Italy') and k != ('Curacao', 'Netherlands') and k != ('Guangdong', 'China') and idx != 9:
    #    continue
    #if k[1] != 'Germany':
    #    continue
    #if k[1] != 'Spain':
    ...
    ret = func_fitting(hist_num, y_scale=max(1000, np.max(hist_num)), b=b_cc,
                       start_pred=60, PN=10, MAXN=len(hist_num)+30)
    ret = list(ret)
    ...
    for i in range(len(pred_cc)):
        index = (test_data_filled['Province/State'] == k[0]) &                 (test_data_filled['Country/Region'] == k[1]) &                 (test_data_filled['Date'] == test_date_list[i])
