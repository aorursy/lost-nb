...
len(category_dict), len(attribute_dict), category_dict, attribute_dict


# In[7]:


len(superattributes_dict), len(supercategories_dict), superattributes_dict, supercategories_dict
...
category2super_dict, attribute2super_dict


# In[10]:


'''trainCSV = os.path.join(dataDir, 'train.csv')
train = pd.read_csv(trainCSV)
train['AttributesIds'] = train['AttributesIds'].fillna(341) # 341 is the `Not Available` class'''
...
train['AttributesIds'] = train['AttributesIds'].apply(str2list)

to_supercategory = lambda val: supercategories_dict[category_dict[val][1]]
train['SuperCategory'] = train['ClassId'].apply(to_supercategory)

to_superattribute = lambda val: np.array([ superattributes_dict[attribute_dict[i][1]] for i in val ])
train['SuperAttribute'] = train['AttributesIds'].apply(to_superattribute)
...
plt.bar(train.ClassId.value_counts().index, train.ClassId.value_counts().values)
for i, val in zip(train.ClassId.value_counts().index, train.ClassId.value_counts().values):
    plt.text(i, val+3000, str(val), rotation=90)'''
...
plt.bar(train.SuperCategory.value_counts().index, train.SuperCategory.value_counts().values)
for i, val in zip(train.SuperCategory.value_counts().index, train.SuperCategory.value_counts().values):
    plt.text(i-0.4, val+1000, str(val))'''


# In[16]:


#pd.read_csv('/kaggle/input/kernel3bda963838/ImageData.csv').to_csv('ImageData.csv', index=False)


# In[17]:


'''def imgs2img(ImageId):
    imgs = train[train.ImageId == ImageId]
    ImgDict = {}
    ImgDict['ImageId'] = ImageId
    ImgDict['EncodedPixels'] = ''
    ImgDict['Height'] = imgs.iloc[0].Height
    ImgDict['Width'] = imgs.iloc[0].Width
    ImgDict['PixelRatio'] = ImgDict['Height'] / ImgDict['Width']
    ImgDict['If3Channels'] = True if len(np.array(Image.open(os.path.join(dataDir, 'train', ImageId+'.jpg'))).shape)==3 else False
    ImgDict['ClassIds'] = []
    ImgDict['SuperCategories'] = []
    
    for i in range(len(imgs)):
        row = imgs.iloc[i]
        ImgDict['EncodedPixels'] = ImgDict['EncodedPixels'] + ' ' + row.EncodedPixels
        ImgDict['ClassIds'].append(row.ClassId)
        ImgDict['SuperCategories'].append(row.SuperCategory)
    ImgDict['EncodedPixels'] = ImgDict['EncodedPixels'][1:]
    ImgDict['SuperCategories'] = list(set(ImgDict['SuperCategories']))
    
    return pd.Series(ImgDict)
tqdm.pandas()
train_image = pd.Series(train.ImageId.unique()).progress_apply(imgs2img)'''


# In[18]:


#train_image.to_csv('ImageData.csv', index=False)
...
'''sortedRatios = train_image.PixelRatio.value_counts().sort_index()  ##Height/Width
hist, bin_edges = np.histogram(np.array(sortedRatios.index), 200)
bin1 = bin_edges[0]
for i in range(len(bin_edges[1:])):
    bin2 = bin_edges[i]
    hist[i-1] = sortedRatios.loc[bin1:bin2].values.sum()
    bin1 = bin2'''
...
'''sortedHeight = train_image.Height.value_counts().sort_index()
hist, bin_edges = np.histogram(np.array(sortedHeight.index), 50)
bin_edges = bin_edges.astype('int64')
bin1 = bin_edges[0]
for i in range(len(bin_edges[1:])):
    bin2 = bin_edges[i]
    hist[i-1] = sortedHeight.loc[bin1:bin2].values.sum()
    bin1 = bin2'''
...
'''maximas = argrelextrema(hist, np.greater)[0]

plt.figure(figsize=(10,7))
plt.plot(bin_edges[:-1], hist);
plt.plot(bin_edges[maximas], hist[maximas], 'o')
for point in maximas:
    plt.text(bin_edges[point], hist[point], str(bin_edges[point]))'''


# In[25]:


SuperCategoryImage = pd.read_csv('/kaggle/input/basereq/SuperCategoryImage.csv')


# In[26]:


'''def imgs2supercategory(ImageId):
    row = train_image[train_image.ImageId == ImageId].iloc[0]
    imgs = train[train.ImageId == row.ImageId]
    df_imgs = []
    
    SuperCats = row.SuperCategories
    for cats in SuperCats: 
        ImgDict = {}
        ImgDict['ImageId'] = row.ImageId
        ImgDict['Height'] = int(row.Height)
        ImgDict['Width'] = int(row.Width)
        ImgDict['SuperCategory'] = int(cats)
        ImgDict['EncodedPixels'] = ''
        ImgDict['If3Channels'] = row.If3Channels
        img = imgs[imgs.SuperCategory == int(cats)]
        for i in range(len(img)):
            ImgDict['EncodedPixels'] = ImgDict['EncodedPixels'] + ' ' + img.iloc[i].EncodedPixels
        df_imgs.append(pd.Series(ImgDict))
    
    return pd.concat(df_imgs, axis=1).transpose()

tqdm.pandas()
SuperCategoryImageList = train_image.ImageId.progress_apply(imgs2supercategory)
SuperCategoryImage = pd.concat(SuperCategoryImageList.values).reset_index().drop(['index'], axis=1)'''


# In[27]:


#SuperCategoryImage.to_csv('SuperCategoryImage.csv', index=False)
...
get_ipython().system('cp -avr /kaggle/input/maskrcnn16 /kaggle/working/Mask_RCNN')
...
SuperCategoryImagesGrouped = SuperCategoryImage.groupby('ImageId')['EncodedPixels', 'SuperCategory', 'Height', 'Width'].agg(lambda x: list(x))
...
max_num = 0
for li in SuperCategoryImagesGrouped.SuperCategory.values:
    if max_num < len(li):
        max_num = len(li)
max_num
...
class FashionDataset(utils.Dataset):
    ...
    def load_mask(self, image_id):
        ...
        for m, (annotation, label) in enumerate(zip(info['annotations'], info['labels'])):
            ...
            annotation = [int(x) for x in annotation.split(' ')[1:]]
...
'''model = modellib.MaskRCNN(mode='training', config=config, model_dir='/kaggle/working/')
...
model_path = '/kaggle/input/trainedmasksupercat1/All1.h5'
print('Loading Weights: All1.h5')
model.load_weights(model_path, by_name=True)

history = pickle.load(open('/kaggle/input/trainedmasksupercat1/All1.pkl', 'rb'))
...
'''%cd ..
model.keras_model.save_weights('Head.h5')
pickle.dump(history, open('Head.pkl', 'ab'))
%cd Mask_RCNN'''
...
'''%cd ..
model.keras_model.save_weights('All1.h5')
pickle.dump(history, open('All1.pkl', 'ab'))
%cd Mask_RCNN'''
...
model.keras_model.save_weights('All2.h5')
pickle.dump(history, open('All2.pkl', 'ab'))
...
get_ipython().run_line_magic('cd', '..')
model.keras_model.save_weights('All3.h5')
pickle.dump(history, open('All3.pkl', 'ab'))
get_ipython().run_line_magic('cd', 'Mask_RCNN')
...
model_path = '/kaggle/working/All2.h5'
