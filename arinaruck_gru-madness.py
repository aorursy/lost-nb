...
test_df = pd.read_csv(data_path + 'quora-insincere-questions-classification/' + 'test.csv')
...
for threshold in np.linspace(0, 1, 1000):
    pr = (test_pr > threshold).astype(int)
    f1 = f1_score(val_ans, pr)
    if f1 > max_score:
        max_score = f1
        best_threshold = threshold
print('mu: {}, best threshold: {}, best f1: {}'.format(mu, best_threshold, max_score))
'''
