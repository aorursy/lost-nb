...
unique_buildings = summer_heater_ids.building_id.value_counts()
bad_buildings_ids = unique_buildings[unique_buildings>2500].index
summer_bad = summer_heater_ids[summer_heater_ids.building_id==bad_buildings_ids[0]]
for bid in bad_buildings_ids[1:]:
    summer_bad.append(summer_heater_ids[summer_heater_ids.building_id==bid])
train.drop(summer_bad.index, inplace=True)

cold_water_ids = train.query('(meter == 1                                     & timestamp >= "2016-01-01" & timestamp <= "2016-03-20"                                    & meter_reading > 0)')
unique_buildings = cold_water_ids.building_id.value_counts()
bad_buildings_ids = unique_buildings[unique_buildings>1880].index
winter_bad = cold_water_ids[cold_water_ids.building_id==bad_buildings_ids[0]]
for bid in bad_buildings_ids[1:]:
    winter_bad.append(cold_water_ids[cold_water_ids.building_id==bid])
...
bayRidge = BayesianRidge(n_iter=300, tol=0.001, alpha_1=1e-06, alpha_2=1e-06, lambda_1=1e-06, lambda_2=1e-06)
lr = LinearRegression(fit_intercept=True, normalize=False)
sgd = SGDRegressor(loss='squared_loss', penalty='l2', alpha=0.0001, l1_ratio=0.15, fit_intercept=True, max_iter=1000, tol=0.001)
...
for idx, (train_idx, val_idx) in enumerate(kfold.split(features)):
    ...
    model = StackingRegressor(regressors=(lightgbm, ridge, lasso, bayRidge, lr, sgd),
