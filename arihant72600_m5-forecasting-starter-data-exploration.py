...
itemToValues = defaultdict(list)
for i, v in zip(sellp.id, sellp.sell_price):
    itemToValues[i].append(v)
itemToMeanValue = {}
for i, v in itemToValues.items():
    itemToMeanValue[i] = statistics.mean(v)
...
# There are 3049 total items for sale
# There are 10 different stores
# CA_1, CA_2, CA_3, CA_4, TX_1, TX_2, TX_3, WI_1, WI_2, WI_3
# For each item and store pair we have computed the mean selling price
# For each item and store pair we have the sell data from d_1 to d_1913
print(len(itemToMeanValue))
print(len(set(sellp.id)))
print(len(set(stv.id)))
print(len(set(stv.item_id)))
print(len(set(stv.store_id)))
allStores = list(set(stv.store_id))
allStores.sort()
print(allStores)
...
totalSold = filtered.sum(axis = 1)
totalSold.index = stv.id
totalSoldLog = totalSold.apply(log)
...
TXStores = allStores[4:7]
WIStores = allStores[7:10]

for stateStores in [CAStores, TXStores, WIStores]:
    fig, ax = plt.subplots(1, len(stateStores), figsize=(5 * len(stateStores), 3))
    for store, axi in zip(stateStores, ax):
        storeDf = stv[stv.store_id == store]
        storeDfSums = storeDf[list(stv)[6:]].sum(axis = 0)
        storeDfSums.plot(title = store, ax = axi)
    plt.tight_layout()
    plt.show()
...
for (i, _), (j, _) in zip(itemToMeanValue.items(), totalSoldLog.items()):
    assert(i == j)
plt.title('log of total sold vs mean price')
